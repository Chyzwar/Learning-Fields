...
class UserAnalysis:
    ...
    def find_user_books(self, visitor_uuid):
        """ Find User Books books by user uuid"""
    ...
    def user_visitors_alike(self, visitor_uuid):
        ...
        user_similar = {}
        for key, uuid in user_readers.iteritems():
            for book in uuid.values():
                user_similar[book] = self.find_book_visitors(book).values()[0].values()
        return user_similar
    ...
    def users_alike_sorted(self, visitor_uuid, sorter):
        ...
        user_readership = {}
        for key, user_list in user_similar.iteritems():
            for user in user_list:
                if user in user_readership:
                    user_readership[user] += 1
                else:
                    user_readership[user] = 1
        return sorted(user_readership.items(), key=operator.itemgetter(1), cmp=sorter)[:10]
    ...
    def book_visitors_alike(self, book_uuid):
        ...
        book_similar = {}
        for key, uuid in book_readers.iteritems():
            for user in uuid.values():
                book_similar[user] = self.find_user_books(user).values()[0].values()
        return book_similar
    ...
    def book_alike_sorted(self, book_uuid, sorter):
        ...
        book_readership = {}
        for key, book_list in book_similar.iteritems():
            for book in book_list:
                if book in book_readership:
                    book_readership[book] += 1
                else:
                    book_readership[book] = 1
        return sorted(book_readership.items(), key=operator.itemgetter(1), cmp=sorter)[:10]

    def sorter(self, x, y):
        """Sorting Function"""
        return y - x

    def best_readers(self):
        ...
        reader_popularity = reader_popularity.sort(['event_readtime'],  ascending=False)
        return reader_popularity

    def best_readers_data(self, number_of):
        """ Get Data of most active users """
        readers_time = self.best_readers()[:number_of]
        readers_list = readers_time['event_readtime'].to_dict()
        readers_data = {}
        for key, value in readers_list.iteritems():
